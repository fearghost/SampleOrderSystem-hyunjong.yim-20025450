...
| # | 테스트명 | 시나리오 | 검증 |
|---|---|---|---|
| 1 | `placeOrder_callsService` | 메뉴 1 선택 | `service.placeOrder()` 1회 |
| 2 | `approveOrder_callsService` | 메뉴 2 → 주문 선택 | `service.approveOrder(orderId)` 1회 |
| 3 | `rejectOrder_callsService` | 메뉴 3 → 주문 선택 | `service.rejectOrder(orderId)` 1회 |
