...
// Order.h에 함께 선언 — 직렬화/역직렬화 쌍
std::string statusToString(OrderStatus s);    // enum → "RESERVED" 등
OrderStatus statusFromString(const std::string& s); // "RESERVED" 등 → enum

struct Order {
    std::string orderId;      // 주문번호 (ORD-YYYYMMDD-NNNN) — OrderService가 내부 생성
    std::string sampleId;     // 시료 ID → Sample
    std::string customerName; // 고객명
    int         quantity;     // 주문 수량
    OrderStatus status;
    std::string createdAt;    // 생성 일시 (ISO 8601) — OrderService가 내부 생성
};
```

> `orderId`와 `createdAt`은 `OrderService::placeOrder()` 내부에서 자동 생성된다.  
> 외부(Controller)에서 주입하지 않는다.
...
struct ProductionJob {
    ...
    int         actualQty;  // 실 생산량 = ceil(shortage / (yieldRate × kYieldSafetyFactor))
...
> **`kYieldSafetyFactor = 0.9`**: `OrderService.cpp`에 `constexpr`로 선언된 수율 안전 계수.  
> 이론 수율의 90%만 실현된다고 가정하여 실 생산량을 보정한다.
...
│   ├── Sample.h
│   ├── Order.h                 # enum class OrderStatus + statusToString/statusFromString
│   └── ProductionJob.h
│
...
│   ├── InMemorySampleRepository.h/cpp  # 테스트용 인메모리 구현체
│   ├── InMemoryOrderRepository.h/cpp
│   ├── InMemoryProductionRepository.h/cpp
│   ├── JsonSampleRepository.h/cpp      # JSON 파일 기반 영속성 구현체
...
│   ├── OrderService.h/cpp      # 주문 접수(orderId·createdAt 내부생성)·승인·거절·출고·생산완료
...
│
├── view/                       # 추상 인터페이스 (가상 메서드) + Console 구현체
│   ├── MainView.h / ConsoleMainView.h/cpp
│   ├── SampleView.h / ConsoleSampleView.h/cpp
│   ├── OrderView.h / ConsoleOrderView.h/cpp
│   ├── MonitorView.h / ConsoleMonitorView.h/cpp
│   ├── ProductionView.h / ConsoleProductionView.h/cpp
│   └── ReleaseView.h / ConsoleReleaseView.h/cpp
│
